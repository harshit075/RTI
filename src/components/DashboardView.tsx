      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-black text-primary-navy tracking-tight dark:text-white">
              {isExpertMode ? 'Expert Workspace' : (language === 'en' ? 'Your RTIs' : 'आपके आरटीआई')}
            </h2>
            <p className="text-xs text-slate-500 mt-1">
              {isExpertMode 
                ? 'Advanced analytical workspace with bulk export and timeline auditing tools.'
        ...
        {!isExpertMode && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="flex flex-wrap gap-2 text-xs font-bold">
              <span className="flex items-center gap-1.5 bg-red-50 text-red-800 border border-red-200 px-3.5 py-1.5 rounded-full">
                <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />
                {actionRequiredRTIs.length} Action Required
              </span>
              <span className="flex items-center gap-1.5 bg-amber-50 text-amber-850 border border-amber-200 px-3.5 py-1.5 rounded-full">
                <span className="h-2 w-2 rounded-full bg-amber-400" />
                {activeRTIs.length} Waiting
              </span>
              <span className="flex items-center gap-1.5 bg-emerald-50 text-emerald-800 border border-emerald-250 px-3.5 py-1.5 rounded-full">
                <span className="h-2 w-2 rounded-full bg-emerald-500" />
                {rtis.filter(r => r.status === 'FAA Decision Received' || (r.status === 'Response Received' && !actionRequiredRTIs.includes(r))).length} Completed
              </span>
            </div>
            ...
            {actionRequiredRTIs.length > 0 ? (
              <div className="space-y-3">
                <span className="text-[10px] font-black text-red-650 uppercase tracking-wider block">Action Required</span>
                <div className="rounded-2xl border border-red-200 bg-red-50/20 p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <span className="text-[10px] text-slate-500 font-mono font-bold block">
                      RTI ID: {actionRequiredRTIs[0].registrationNumber || actionRequiredRTIs[0].id}
                    </span>
                    <h4 className="font-bold text-sm text-slate-800 leading-snug mt-1">{actionRequiredRTIs[0].title}</h4>
                    <p className="text-xs text-slate-600 mt-1">
                      CPIO reply has been uploaded. AI review suggests Question 3 regarding structural audits was not fully answered. Recommends filing appeal.
                    </p>
                  </div>
                  <button
                    onClick={() => openRtiDetail(actionRequiredRTIs[0].id)}
                    className="bg-[#B94A48] hover:bg-red-750 text-white font-bold text-xs px-4.5 py-2.5 rounded-xl shrink-0 shadow-sm cursor-pointer whitespace-nowrap w-full sm:w-auto"
                  >
                    Review Response →
                  </button>
                </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 text-xs text-slate-600 leading-relaxed">
                All filed applications are tracking within regular timeline limits. No immediate action required.
              </div>
            )}

            {/* RECENT APPLICATIONS segment */}
            <div className="space-y-3">
              <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">Recent Applications</span>
              
              {rtis.length === 0 ? (
                <div className="text-center py-10 bg-white border border-slate-200 border-dashed rounded-2xl">
                  <Inbox className="h-8 w-8 text-slate-300 mx-auto mb-2" />
                  <p className="text-xs text-slate-500">No RTIs filed yet. Start an application to query records.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {rtis.map(rti => {
                    const isPending = rti.status === 'Submitted' || rti.status === 'Response Pending' || rti.status === 'Processing';
                    return (
                      <div 
                        key={rti.id} 
                        onClick={() => openRtiDetail(rti.id)}
                        className="bg-white hover:bg-slate-50 border border-slate-200 rounded-xl p-4 flex justify-between items-center cursor-pointer transition-colors shadow-3xs"
                      >
                        <div>
                          <h4 className="font-bold text-slate-800 text-xs">{rti.title}</h4>
                          <span className="text-[10px] text-slate-500 font-bold block">{getAuthorityName(rti.authorityId)}</span>
                        </div>
                        <span className={`text-[10px] font-extrabold border px-2 py-0.5 rounded-full ${
                          isPending ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-emerald-50 text-emerald-800 border-emerald-200'
                        }`}>
                          {isPending ? 'Response Pending' : 'Completed'}
                        </span>
                      </div>
                    );
                  })}
        ...
        {isExpertMode && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4.5 flex flex-wrap gap-3 items-center justify-between shadow-2xs">
              <span className="text-xs font-bold text-slate-600">Export active cases and records for records compliance audit:</span>
              <div className="flex gap-2">
                <button
                  onClick={handleExportJson}
                  className="rounded-lg bg-slate-100 hover:bg-slate-250 border border-slate-200 text-slate-700 font-bold px-3 py-2 text-xs transition-colors cursor-pointer"
                >
                  Export JSON
                </button>
                <button
